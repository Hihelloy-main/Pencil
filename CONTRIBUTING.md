...
3. cd into `pencil-server` for server changes, and `penicl-api` for API changes.
**Only changes made in `pencil-server/src/minecraft` have to deal with the patch system.**

`pencil-server/src/minecraft` is not a git repositories in the traditional sense. Its
...
1. cd into `pencil-server/src/minecraft/java` and run `git rebase -i base`;
...
1. Modify `pencil-server/src/minecraft` with the appropriate changes;
